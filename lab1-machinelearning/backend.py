...
alphabet above have been substituted with ASCII symbols.""".strip())
    ...
    def _encode(self, inp_x, inp_y):
        ...
        if(len(inp_x) == 1):
            return torch.nn.functional.pad(torch.tensor(np.array(xs), dtype=torch.float),(0,0,0,0,0,10 - len(xs))), torch.tensor(y, dtype=torch.float)

        return torch.tensor(np.array(xs), dtype=torch.float), torch.tensor(y, dtype=torch.float)
    ...
    def _predict(self, split='test'):
        ...
        # print(f'shapes {[i.clone().detach().shape for i in all_predicted]}')
        all_predicted_probs = [sftmax(i.clone().detach()) for i in all_predicted]
