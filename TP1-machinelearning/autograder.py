...
class Tracker(object):
    def __init__(self, questions, maxes, prereqs, mute_output, gs=False):
    ...
    def finalize(self):
        ...
        if self.gs:
            ...
            with open(RESULTS, 'w') as f:
                d = { 
                    'score': sum(self.points.values()),
                    'output': 'Your score has been recorded. Thank you!'
                }
                json.dump(d, f)
        else:
            print("""
...
def main():
    ...
    tracker = Tracker(questions, maxes, PREREQS, options.mute_output, gs=options.gs_output)
    for q in questions:
...
def check_digit_classification(tracker):
    ...
    for batch_size in (1, 2, 4):
        ...
        # assert grad_x[0] != None, "Node returned from DigitClassificationModel.get_loss() does not depend on the provided input (x)"
        assert grad_y[0] != None, "Node returned from DigitClassificationModel.get_loss() does not depend on the provided labels (y)"
...
def check_lang_id(tracker):
    ...
    for batch_size, word_length in ((1, 1), (2, 1), (2, 6), (4, 8)):
        ...
        inp_xs = inp_xs[:word_length].clone().detach().requires_grad_(True)
...
def check_convolution(tracker):
    ...
    for batch_size in (1, 2, 4):
        ...
        inp_x = dataset[:batch_size]['x'].clone().detach().requires_grad_(True)
        # inp_y = torch.tensor(dataset[:batch_size]['label'], dtype=torch.float, requires_grad=True) 
        inp_y = dataset[:batch_size]['label'].clone().detach().requires_grad_(True)
        loss =  model.get_loss(inp_x, inp_y)

        verify_node(loss, 'tensor', (1,), "DigitConvolutionalModel.run()")

   
        grad_y = torch.autograd.grad(loss, inp_x, allow_unused=True, retain_graph=True)
        grad_x = torch.autograd.grad(loss, inp_y, allow_unused=True, retain_graph=True)
        
        # assert grad_x[0] != None, "Node returned from DigitConvolutionalModel.get_loss() does not depend on the provided input (x)"
        assert grad_y[0] != None, "Node returned from DigitConvolutionalModel.get_loss() does not depend on the provided labels (y)"
...
def check_attention(tracker):
    ...
    for block_size in [2,4,16]:
        ...
        expected_output = expected_output.masked_fill(att_block.mask[0,0,:T,:T] == 0, float('-inf'))
